...
export default function ForgotEmailPage() {
  ...
  const handleRecoverEmail = async (e: React.FormEvent) => {
    ...
    try {
      const res = await MockAPI.forgotEmail(username);
      setEmail(res.email);
    } catch (err: any) {
      ...
      <div className="glass-panel w-full max-w-md p-8 rounded-3xl border border-violet-500/10">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 light:text-slate-500 text-sm">
            Enter your username to find the email address associated with your account
          </p>
        </div>
        ...
        {email ? (
          <div className="mb-6 p-6 rounded-2xl bg-violet-500/10 border border-violet-500/20 text-center animate-slide-up">
            <p className="text-gray-400 text-sm mb-2">Registered Email Address:</p>
            <p className="text-xl font-bold text-white tracking-wide select-all">{email}</p>
            <p className="text-xs text-gray-500 mt-4">
              You can now use this email address to log in or reset your password.
            </p>
          </div>
        ) : (
          <form onSubmit={handleRecoverEmail} className="flex flex-col gap-5">
